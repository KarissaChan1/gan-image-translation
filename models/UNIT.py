...
from tensorflow.keras.utils import save_img, array_to_img
import numpy as np
import json
import sys
import time
...
class UNIT():
    ...
    def __init__(self, lr = 1e-4, date_time_string_addition=''):
        self.channels = 1 # 1 for grayscale 3 RGB
        ...
        nr_A_test_imgs = None
        nr_B_test_imgs = None
        image_folder = data_path #'dataset-name/'
        ...
        if self.use_data_generator:
            ...
            self.A_train = self.data_generator[0] #trainA_images
            self.B_train = self.data_generator[1] #trainB_images
            self.A_test = self.data_generator[2] #testA_images
            self.B_test = self.data_generator[3] #testB_images

            print('Length self.A_train: ',self.A_train.__len__())
            print('Length self.B_test: ',self.B_test.__len__())
            print('Length self.data_generator: ',self.data_generator.__len__())
        ...
        self.img_width = 256 #self.A_train.shape[2]
        self.img_height = 256 #self.A_train.shape[1]
        ...
        self.discriminatorA.compile(optimizer=self.opt,
                                    ...
                                    loss_weights=[self.lambda_0,
                                                  self.lambda_0,
                                                  self.lambda_0])
        self.discriminatorB.compile(optimizer=self.opt,
                                    loss=['binary_crossentropy',
                                          'binary_crossentropy',
                                          'binary_crossentropy'],
                                    loss_weights=[self.lambda_0,
        ...
        self.encoderGeneratorModel.compile(optimizer=self.opt,
...
# Models
    ...
    def modelSimple(self):
        ...
        return Model(inputs=inputImg, outputs=prediction)

#===============================================================================
# Training

    def trainFullModel(self, outputdir, epochs=100, batch_size=1, save_interval=1,resume=False):
        ...
        def run_training_iteration(loop_index, epoch_iterations, imgA, imgB):
            ...
            if loop_index % epoch_iterations == 0:
                ...
                self.save_tmp_images(outputdir, imgA, imgB)
        ...
        self.writeMetaDataToJSON(outputdir)
        # self.saveImages(outputdir,'init', 1)
        sys.stdout.flush()
        ...
        if resume==True:
            start_epoch = self.load_checkpoints(outputdir) + 1
        else:
            start_epoch=0
        ...
        for epoch in range(start_epoch,epochs):
            if self.use_data_generator:
                loop_index = 0
                for i in range(np.min((self.A_train.__len__(),self.B_train.__len__()))):
                    imgA = self.A_train[i]
                    imgB = self.B_train[i]
  
                    # Run all training steps
                    epoch_iterations = np.min((self.A_train.__len__(),self.B_train.__len__()))
                    run_training_iteration(loop_index, epoch_iterations, imgA, imgB)
                    print("-----------------Loop Index:", loop_index)
  
                    if loop_index >= epoch_iterations:
                        break
                    loop_index += 1
            ...
            if epoch % save_interval == 0:
                ...
                self.saveImages(outputdir,epoch, 10)
            ...
            if epoch==(epochs-1):
                ...
                self.saveModel(outputdir,self.generatorB, 'generatorB', epoch)
                self.saveModel(outputdir,self.encoderA, 'encoderA', epoch)
                self.saveModel(outputdir,self.encoderB, 'encoderB', epoch)
                self.saveModel(outputdir,self.decoderShared, 'decoderShared', epoch)
                self.saveModel(outputdir,self.encoderShared, 'encoderShared', epoch)
        sys.stdout.flush()

    def saveCurrentModels(self, outputdir, epoch):
        self.saveModel(outputdir,self.discriminatorA, 'discriminatorA', epoch)
        self.saveModel(outputdir,self.discriminatorB, 'discriminatorB', epoch)
        self.saveModel(outputdir,self.generatorA, 'generatorA', epoch)
        self.saveModel(outputdir,self.generatorB, 'generatorB', epoch)
    ...
    def trainSimpleModel(self, epochs=200, batch_size=1, T=1):
        ...
        self.saveImagesSimpleModel(outputdir,epoch=epochs, num_saved_images=10, T=T)
        self.saveModel(outputdir,self.superSimple, 'superSimpleModel', epochs)

#===============================================================================
# Save and load Models
    def load_checkpoints(self,outputdir):

      directory = os.path.join(outputdir,'checkpoints')
      ckpt = Checkpoint(encoderA = self.encoderA,
                          encoderB = self.encoderB,
                          gen_A = self.generatorA,
    ...
    def saveCheckpoints(self, outputdir, epoch):
        directory = os.path.join(outputdir,'checkpoints')
    ...
    def saveModel(self, outputdir, model, model_name, epoch):
        directory = os.path.join(outputdir+'saved_model', self.date_time)
        if not os.path.exists(directory):
            os.makedirs(directory)

        model_path_w = outputdir+'saved_model/{}/{}_epoch_{}_weights.hdf5'.format(self.date_time, model_name, epoch)
        model.save_weights(model_path_w)
        model_path_m = outputdir+'saved_model/{}/{}_epoch_{}_model.json'.format(self.date_time, model_name, epoch)
        model.save_weights(model_path_m)

    def loadAllWeightsToModelsIncludeDisc(self, outputdir, folder_name, epoch):
        pathEncoderA = outputdir+'saved_model/{}/encoderA_epoch_{}_weights.hdf5'.format(folder_name, epoch)
        pathEncoderB = outputdir+'saved_model/{}/encoderB_epoch_{}_weights.hdf5'.format(folder_name, epoch)

        pathEncoderShared = outputdir+'saved_model/{}/encoderShared_epoch_{}_weights.hdf5'.format(folder_name, epoch)
        pathDecoderShared = outputdir+'saved_model/{}/decoderShared_epoch_{}_weights.hdf5'.format(folder_name, epoch)

        pathGeneratorA = outputdir+'saved_model/{}/generatorA_epoch_{}_weights.hdf5'.format(folder_name, epoch)
        pathGeneratorB = outputdir+'saved_model/{}/generatorB_epoch_{}_weights.hdf5'.format(folder_name, epoch)

        pathDiscriminatorA = outputdir+'saved_model/{}/discriminatorA_epoch_{}_weights.hdf5'.format(folder_name, epoch)
        pathDiscriminatorB = outputdir+'saved_model/{}/discriminatorB_epoch_{}_weights.hdf5'.format(folder_name, epoch)
    ...
    def loadAllWeightsToModels(self, outputdir, folder_name, epoch):
        pathEncoderA = outputdir+'saved_model/{}/encoderA_epoch_{}_weights.hdf5'.format(folder_name, epoch)
        pathEncoderB = outputdir+'saved_model/{}/encoderB_epoch_{}_weights.hdf5'.format(folder_name, epoch)
    ...
    def load_model_and_generate_synthetic_images(self, outputdir, folder_name, epoch):
        ...
        def save_image(outputdir,image, name, domain):
            if self.channels == 1:
                image = image[:, :, 0]
            image = np.clip(image/2 + 0.5, 0, 1)

            directory = os.path.join(outputdir,'generate_images_test', folder_name, domain)
            if not os.path.exists(directory):
                os.makedirs(directory)
    ...
    def truncateAndSave(self, outputdir, real_A, real_B, synthetic, reconstructed, epoch, sample, name, filename, tmp=False):
        synthetic = synthetic.clip(min=-1)
        if reconstructed is not None:
            reconstructed = reconstructed.clip(min=-1)
        # Append and save
        if tmp:
             save_img(outputdir+'images/{}/{}.png'.format(
                self.date_time, name), synthetic)
        else:
            if real_A is None and real_B is None:
                save_img(outputdir+'images/{}/{}/{}_synt.png'.format(
            ...
            np.save(outputdir+'images/{}/{}/epoch{}_sample{}.npy'.format(self.date_time, name, epoch, sample), image)
            # save_img(outputdir+'images/{}/{}/epoch{}_sample{}.jpg'.format(self.date_time, name, epoch, sample), image)
            # save_img(outputdir+'images/{}/{}/epoch{}_sample{}_{}.png'.format(self.date_time, name, epoch, sample,filename), image)

    def saveImages(self, outputdir, epoch, num_saved_images=1):

        directory = os.path.join(outputdir,'images', self.date_time)
        ...
        for i in range(num_saved_images):
            ...
            if self.channels == 1:
                ...
                self.truncateAndSave(outputdir, imgA, imgB, outAb0, outC_Ba, epoch, i, 'B','A2B', tmp=False)
                self.truncateAndSave(outputdir, imgB, imgA, outBa0, outC_Ab, epoch, i, 'A', 'B2A',tmp=False)
            else:
                imgA = imgA[0, :, :, :]
                outAb0 = outAb[0, :, :, :]
                outAa0 = outAa[0, :, :, :]
                ...
                self.truncateAndSave(outputdir,None, imgA, outAb0, outC_Ba, epoch, i, 'A', None)
                self.truncateAndSave(outputdir,None, imgB, outBa0, outC_Ab, epoch, i, 'B', None)

    def saveImagesSimpleModel(self, outputdir,epoch, num_saved_images=1, T=1):
        directory = os.path.join(outputdir,'images', self.date_time)
        if not os.path.exists(os.path.join(directory, 'A')):
            os.makedirs(os.path.join(directory, 'A'))
            os.makedirs(os.path.join(directory, 'B'))
    ...
    def save_tmp_images(self, outputdir, imgA, imgB):
        try:
            # Generate fake images
            encodedImageA = self.encoderA.predict(imgA)
            encodedImageB = self.encoderB.predict(imgB)
            ...
            np.save(outputdir+'images/{}/{}.npy'.format(self.date_time, 'tmp'), image_tot)
            save_img(outputdir+'images/{}/{}.png'.format(self.date_time, 'tmp'), image_tot_clip)
    ...
    def writeLossDataToFile(self,outputdir):
        keys = sorted(self.history.keys())
        with open(outputdir+'images/{}/loss_output.csv'.format(self.date_time), 'w') as csv_file:
            writer = csv.writer(csv_file, delimiter=',')
            writer.writerow(keys)
            writer.writerows(zip(*[self.history[key] for key in keys]))

    def writeMetaDataToJSON(self,outputdir):
        directory = os.path.join(outputdir,'images', self.date_time)
        if not os.path.exists(directory):
            os.makedirs(directory)
        # Save meta_data
        ...
        with open(outputdir+'images/{}/meta_data.json'.format(self.date_time), 'w') as outfile:
